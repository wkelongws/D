...
output_single_sensor = True
use_volume_and_occup = False
use_weather = False
use_history = True
input_feature = 'cnn'
weights_file_name = 'Speedonly_with_history_single_sensor'
batch_size = 1000
epochs = 10000
...
#         printdb_trans_1 = Dataset_NVIDIA_1(annotation_list,frame_list,transform = PerspectiveTransform(MAP_FILE, world_origin, pixel_ratio, PMAT_FILE))(self.__len__())
    ...
    def __getitem__(self, idx):
        ...
        Weather_today = self.Weather[idx,:,:-1,:]
        Traffic_history = 0 
        Traffic_output = self.Traffic[idx,2:3,1:,0]
...
#         assert input_feature in ['raw','linear','cnn'], 'wrong choice for input_feature'
        ...
        if self.input_feature == 'cnn':
            ...
            self.CNN_feature_extract = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(filter_height,1)),nn.BatchNorm2d(1),nn.ReLU())
            self.CNN_for_history = self.CNN_feature_extract
            if self.use_volume_and_occup:
                self.CNN_feature_extract = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(filter_height,3)),nn.BatchNorm2d(1),nn.ReLU())
            if self.use_weather:
                self.CNN_feature_extract = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(filter_height,12)),nn.BatchNorm2d(1),nn.ReLU())
        self.CNN_today_and_history_output_combine = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(1,2)),nn.BatchNorm2d(1),nn.ReLU())
